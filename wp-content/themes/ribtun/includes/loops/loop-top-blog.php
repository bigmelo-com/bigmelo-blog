	...
	<div class="topBlog">	
		...
		<?php if(ribtun_globals('display_post_meta')) { ?>
			<div class = "post-meta">
				...
				<?php if(is_single()) { ?>
					<a class="post-meta-author" href="<?php echo  the_author_meta( 'user_url' ) ?>"><?php esc_attr_e('by ','ribtun'); echo get_the_author(); ?></a>
				<?php } ?>	
				<?php if(empty($postmeta["ribtun_sigle_option_recipe"][0])){ ?>
					<a href="<?php echo the_permalink() ?>#comments"><?php comments_number(); ?></a>		
				<?php } else { ?>
					<?php echo '<a class="recipe-rating" href="'. esc_url(get_the_permalink()) .'#comments">' . esc_attr__('Recipe rating: ','ribtun')  . ribtun_recipe('wprm_rating').'</a>' ; ?>
				<?php } ?>		
